...
|:-------------------------------------|:-----------------|:---------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | 0x181810         | 0x181810       | 0x181850             |
| `newData` 地址                       | 尚未申请，不填写 | 0x181850       | 已离开作用域，不填写 |
| 当前对象的 `size`                    | 4                | 4              | 5                    |
...
> 在此填写你的回答（不少于 20 字）。
答：不可以访问。1. `delete[]` 作用是把这块堆内存交还给操作系统堆管理器，内存已经被释放，这块内存属于无效内存。
调试器看到原来的数据只是内存中残留内容，内存不再归本程序管理。访问已经释放的内存属于野指针，会触发未定义行为，可能篡改其他数据、造成程序崩溃。
...
> 在此填写你的回答（不少于 30 字）。
答：会导致原数据被销毁，数据丢失，
`size`代表当前**实际有效元素数量**，`capacity`是总可用空间。capacity 变大只是多分配空闲内存，
不代表多出来的位置已经存了有效数据，所以不能直接把 size 设置等于 capacity。
---
