...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x???????? | 0x???????? | 0x???????? |
| `newData` 地址 | 尚未申请，不填写 | 0x???????? | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[]` 执行后该数组的内存已归还运行时系统，继续访问旧地址属于未定义行为，读到残留字节只是因为内存未被立即覆盖，并不代表数组仍然有效，可能导致数据错误或程序崩溃，因此程序内绝不能再去访问已释放的旧数组。
...
> 扩容时旧数组是数据唯一的存放处，必须先申请新数组并把现有元素复制过去，保证信息不丢失，然后才能释放旧数组，否则数据会全部丢失。`capacity` 表示数组能容纳的元素空间上限，`size` 表示当前实际存储的有效元素个数，二者含义不同；如果直接把 `size` 设成 `capacity`，会把尚未赋值的空位当成有效元素，导致读出未初始化或残留数据。两倍扩容只是预留空间，并不代表立刻就有那么多有效元素。
