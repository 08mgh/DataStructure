...
> 在此填写你的回答（不少于 30 字）必须先复制再释放，是因为一旦释放旧数组，原数据就会丢失且指针悬垂，导致无法读取数据进行迁移。扩容后不能直接将 size 设为 capacity，是因为 size 代表实际存储的有效元素个数，而新增的容量区域尚未存入有效数据，强行修改 size 会导致读取到未初始化的垃圾值。。
