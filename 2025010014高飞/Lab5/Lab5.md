...
扩容时必须先复制再释放，因为释放旧数组后数据就丢失了，无法复制。扩容只是增加容量，并不增加有效元素个数，size 只在插入新元素时才加一，把 size 设为 capacity 会导致未初始化的内存被当作有效元素。
...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | （填写实际地址） | （填写实际地址） | （填写实际地址） |
| `newData` 地址 | 尚未申请，不填写 | （填写实际地址） | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
不可以。delete[] 释放了内存所有权，之后访问属于未定义行为，内存可能被重新分配、覆盖或标记为不可访问，残留字节不代表数据仍有效。