...
当前 data 地址（图 C 为 v.data）	0x0000000001051940	0x0000000000fa4450	0x0000000000fa4450
newData 地址	尚未申请，不填写	0x0000000001054450	已离开作用域，不填写
当前对象的 size	0	 4	5
...
不可以。delete[] 释放内存后，该指针变为悬空指针，内存管理权已交还系统。虽然内存视图中可能残留旧数据（未被覆盖），但该区域随时可能被重新分配给其他变量，此时继续访问属于未定义行为，会导致程序崩溃或数据错误。
