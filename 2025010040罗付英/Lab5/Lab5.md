...
| :--- |:--------------------|:--------------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | 0x000000000001e1920 | 0x000000000001e1920 | 0x00000000000f81960  |
| `newData` 地址 | 尚未申请，不填写    | 0x00000000000f81960 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4                   | 4                   | 4                    |
...
> 在此填写你的回答（不少于 20 字）。
不可以，delete[]已释放该堆内存，虽还能看到残留数据，访问会产生未定义行为。
#### 3.6.5 截图检查标准
...
> 在此填写你的回答（不少于 30 字）。
如果先释放旧数组，原有数据就会丢失，因此要先把旧数组的有效元素复制到新数组，再释放新数组内存。size代表有效元素数量，capacity是总存储容量，扩容只是增加存储空间，并不会新增有效元素。
---
