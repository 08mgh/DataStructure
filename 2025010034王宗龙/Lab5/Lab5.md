...
> 扩容时必须先复制再释放旧数组，因为旧数组释放后其内存区域可能立即失效或被重用，此时再读取旧数组的数据会导致未定义行为。扩容后不能把 `size` 设成 `capacity`，因为 `size` 表示当前有效元素个数，而新数组中只有前 `size` 个位置是有效数据，其余位置是未初始化的空位，应当由后续插入操作逐个填入，而非一次性标记为全部有效。
