...
> 不可以继续访问旧数组。`delete[]` 执行后，旧数组所占的内存已被系统回收，该地址可能已被其他程序或数据占用。继续通过旧指针访问属于未定义行为，可能导致程序崩溃或数据损坏。Memory View 显示的残留字节只是调试器的缓存，并不代表内存仍然有效。
...
> 扩容时必须先复制再释放旧数组，因为一旦释放旧数组，其内存可能被系统回收或覆盖，无法再读取旧数据进行复制。扩容后不能把 `size` 设为 `capacity`，因为 `expand()` 只负责申请更大的空间，`size` 应当只在实际插入新元素时由 `insert` 增加，二者语义不同：`capacity` 表示可用空间总量，`size` 表示已存储的元素个数。
