...
> 不能。`delete[]` 已经把旧数组的内存交还给堆分配器，这块内存已失效，程序继续访问它是未定义行为，可能读到被覆盖或重复使用的数据，甚至导致程序崩溃；调试器显示的残留字节只是内存未被改写时留下的旧内容，不代表旧数组仍然有效。
...
> 扩容时如果先释放旧数组，再申请新数组并复制，旧元素已经丢失，复制就无从谈起；所以必须先用新数组暂存全部有效元素，确认复制完成后才能释放旧数组，以保证任何时刻数据都不会丢失。而 `size` 表示真正存入的有效元素个数，扩容只是把存储空间扩大，元素并没有自动变多，`size` 只能由插入操作逐个增加，不能直接设为 `capacity`。
